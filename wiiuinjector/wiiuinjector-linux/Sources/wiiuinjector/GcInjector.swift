...
struct GcInjector {
    ...
    func inject(rom: String, iconTex: String, bootTvTex: String, titleId: String, titleKey: String, name: String) throws -> Bool {
        ...
        do {
            ...
            try wit.run()
            wit.waitUntilExit()
            try filem.removeItem(atPath: "\(tmpDir)/gcbase")
        }catch {
            print(error)
        }
        ...
        do {
            ...
            try wit.run()
            wit.waitUntilExit()
            try filem.removeItem(atPath: "\(tmpDir)/game.iso")
        }catch {
            print(error)
        }
        ...
        do {
            ...
            nfs2iso2nfs.arguments = [ Bundle.main.resourcePath! + "/tools/nfs2iso2nfs", "-enc", "-homebrew"]
